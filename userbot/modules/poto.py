...
async def potocmd(event):
    ...
    await event.edit(f"`Sedang Di Prosess........`")
    if user:
    ...
    if id.strip() == "":
        if len(photos) > 0:
            ...
            await event.edit(f"`Poto Berhasil Di Unggah`")
        else:
            try:
                if u is True:
                ...
                await event.edit("**Pengguna Ini Tidak Memiliki Foto**")
                return
    else:
        try:
            id = int(id)
            if id <= 0:
                await event.edit("`ID Nomor Valid` **Apakah Kamu Bercanda Padaku ?**")
                return
        except BaseException:
            await event.edit(f"`Apakah Kamu Bercanda Padaku ?`")
            return
        if int(id) <= (len(photos)):
            send_photos = await event.client.download_media(photos[id - 1])
            await event.client.send_file(event.chat_id, send_photos)
        else:
            await event.edit(f"`Tidak Ada Poto Yang Di Temukan!`")
